...
```bash
end=$((SECONDS+120)); while [ $(grep -c 'SIG:.*:@Gemini\|SIG:.*:@ALL\|SIG:Lead:DEBATE:CLOSE\|SIG:Lead:DEBATE:REDIRECT\|SIG:Lead:DEBATE:INTENT_CLOSE\|SIG:Lead:PHASE:CONVERGE' CHATROOM) -le CURRENT_COUNT ]; do [ $SECONDS -ge $end ] && break; sleep 2; done; echo "WAIT DONE"
```
...
⚠️ **DUPLICATE PREVENTION**: Each phase response (DIVERGE, CHALLENGE, CONVERGE) must be written EXACTLY ONCE. Before writing, grep for your SIG — if it already exists, DO NOT write it again. If you see your SIG:Gemini:CHALLENGE already in the chatroom but the latest Lead SIG is DEBATE:OPEN, that means you've moved past CHALLENGE — go to the DEBATE:OPEN row, not CHALLENGE.
...
| DEBATE:OPEN | No new @Gemini, msg_count≥50 or nothing to say | Write DONE → Shell: append → Shell: wait for CONVERGE → go to Step 1 |
...
| PHASE:CONVERGE | No SIG:Gemini:CONVERGE | Compose (with optional DISSENT block) → Write tmp → Shell: append → go to Step 1 |
...
- **Converge**: Synthesize all perspectives into something genuinely new. Use DISSENT block only for genuinely unresolved disagreements with evidence.
