...
本项目提供两个版本的 Agent 定义文件，按需选择：

### V1 — 原始版（`v1/agents/`）

核心 5 阶段流程，简洁、经过充分测试、可靠。

- 5 阶段：DIVERGE → CHALLENGE → FREE DEBATE → CONVERGE → SYNTHESIS
- Lead 单方面控制辩论收口（`DEBATE:CLOSE`）
- 每个参与者最多 8 条辩论消息
- 无少数派报告机制

**适合**：快速头脑风暴、简单话题、最小化开销。

### V2 — 增强版（`v2/agents/`）⭐ 推荐

在 V1 基础上引入三个高杠杆改进，灵感来自[《罗伯特议事规则》](https://zh.wikipedia.org/wiki/%E7%BD%97%E4%BC%AF%E7%89%B9%E8%AE%AE%E4%BA%8B%E8%A7%84%E5%88%99)（1876）。核心哲学：*"The great purpose of all rules and forms, is to subserve the will of the assembly, rather than to restrain it."*

**V2 新增：**

1. **程序性权利 + Lead RULING**：参与者可在任何阶段用自然语言质疑讨论方向或补充遗漏议题。Lead **必须**主动扫描这些关切并给出正式 RULING（纳入/拒绝 + 理由）。建立"从下到上"的反馈通道——参与者不再被 Lead 的议程锁死。

2. **静默同意窗口**：Lead 不再单方面收口。改为：广播 `INTENT_CLOSE` → 等待异议（必须附带新证据）→ 然后才 `CLOSE`。防止有价值的讨论被过早截断。

3. **DISSENT（少数派报告）**：参与者在 CONVERGE 阶段可附带可选的 `DISSENT:` 块（含证据/推理链）。Lead **必须**在最终综合中保留未被反驳的少数派观点。防止伪共识——AI 的分歧往往代表有价值的不同视角。

**V2 同时包含：**
- 每个参与者最多 50 条辩论消息（放开讨论上限）
- 防重复提交机制（修复状态机 bug）
- 主席中立原则（Lead 在 DIVERGE→DEBATE 期间只管程序，将实质分析留给 SYNTHESIS）

**适合**：复杂话题、深度分析、高利害决策——需要充分的对抗性检验。
...
DIVERGE ──▶ CHALLENGE ──▶ FREE DEBATE ──▶ CONVERGE ──▶ SYNTHESIS
  发散         挑战          自由辩论         收敛          综合
                                │
                    V2: INTENT_CLOSE → 静默同意窗口 → CLOSE
```

| 阶段 | Lead 做什么 | 参与者做什么 |
|------|-----------|-----------|
| **DIVERGE** | 提出议题 + 3-4 个子问题 | 各自独立给出观点；可用自然语言指出遗漏议题（V2） |
| **CHALLENGE** | 提炼冲突点，点名挑战 | 逐点 AGREE/DISAGREE；可用自然语言质疑方向（V2） |
| **FREE DEBATE** | 开启辩论 + 当裁判 | 互相 @对方 直接辩论，可搜索证据 |
| **静默同意窗口** | 广播 INTENT_CLOSE，等待异议（V2） | 有新证据可 OBJECTION，否则默认同意 |
| **CONVERGE** | 给出共识草案 | 确认 + 可选 DISSENT 块附带证据（V2） |
| **SYNTHESIS** | 综合全部讨论（V2 中保留少数派观点） | — |
...
**最简方式**：直接把本仓库链接 `https://github.com/hyleepp/multiagent-brainstorm` 发给 Cursor Agent，它会自动识别并完成安装（默认安装 V2）。

**手动安装**：选择版本，复制 4 个 Agent 文件：

```bash
# V2（推荐）— 含程序性权利、静默同意窗口、DISSENT
...
# 全局安装（所有项目可用）— 改用 ~/.cursor/agents/
mkdir -p ~/.cursor/agents
cp v2/agents/brainstorm-*.md ~/.cursor/agents/   # 或 v1/agents/
```
...
编辑 `v1/agents/brainstorm-*.md` 或 `v2/agents/brainstorm-*.md` 中的 `Debate Style` 部分。当前角色分配基于各模型的认知特长：
...
Agent 默认跟随用户话题的语言输出。如需强制指定语言，修改每个文件的 `Rules` 部分中的 `Respond in the same language as the user's topic` 即可。
...
**V2：**
- `Max 50 debate messages`：每个参与者在 FREE DEBATE 阶段最多发 50 条消息
- Lead 的 `Max 50 loops on Step 10`：裁判最多等 50 轮再强制收口
- 静默同意窗口：60s（可在 Lead prompt 中调整）
...
| 辩论阶段卡住 | grep 未监听 REDIRECT | wait 命令已包含 `DEBATE:REDIRECT`（V2 还包含 `INTENT_CLOSE`） |
| 阶段回复重复 | Agent 在 DEBATE 阶段又提交了 CHALLENGE | V2 新增 DUPLICATE PREVENTION 规则 |
| INTENT_CLOSE 后跳 CONVERGE | Agent 把 INTENT_CLOSE 当成 PHASE:CONVERGE | V2 在 Action Table 中加了显式警告 |
| 某个 Agent 未响应 | 网络/超时 | Lead 超时兜底，2/3 响应即可推进 |

## 开发历程

经过约 27 轮迭代测试，解决了以下核心工程挑战：
...
7. **程序性权利（V2）**：借鉴《罗伯特议事规则》——RULING 机制、静默同意窗口、DISSENT 少数派报告、主席中立原则
